...
function MarkdownRenderer({ content, style }) {
  ...
  useEffect(() => {
    ...
    // MathJax owns these children. Reapplying dangerouslySetInnerHTML on a
    // playback rerender would replace its SVG output with the original TeX.
    element.innerHTML = renderedContent;
    const script = document.getElementById('MathJax-script');
    let cancelled = false;
    const typeset = async () => {
      const mathJax = window.MathJax;
      if (!renderedContent || !element || !mathJax?.startup?.promise) return;
      try {
...
function BilibiliRenderer({ rendering }) {
  const [loaded, setLoaded] = useState(false);
  return (
    <span style={{ display: 'inline-block', width: 800, maxWidth: '100%', ...rendering.style }}>
      {loaded ? (
        <iframe src={rendering.data} title="B站视频播放器"
          style={{ display: 'block', width: '100%', aspectRatio: '16 / 9', border: 0 }}
          allow="fullscreen; picture-in-picture" allowFullScreen />
      ) : (
        <button onClick={() => setLoaded(true)}>加载 B站播放器</button>
      )}
      <a href={rendering.external_link.url} target="_blank" rel="noreferrer">在 B站打开</a>
    </span>
  );
}
...
function VideoRenderer({ src, style, playbackKey }) {
  ...
  useEffect(() => {
    ...
    // Pause on step/mode changes and unmount without resetting the position.
...
function TraceViewer() {
  ...
  const tracePath = urlParams.get('trace') || 'var/traces/lecture_01.json';  // JSON file that has everything
    ...
    if (!rawMode && renderings && renderings.length > 0) {
      ...
      const renderedRenderings = renderings.map((rendering, index) => {
        return <span key={index}>
          {renderRendering(rendering, navigate, `${currentPath}:${currentStepIndex}:${animateMode}`)}
        </span>;
      });
...
function renderRendering(rendering, navigate, playbackKey) {
  if (rendering.type === "markdown") {
    ...
    return <BilibiliRenderer key={`${rendering.data}:${playbackKey}`} rendering={rendering} />;
